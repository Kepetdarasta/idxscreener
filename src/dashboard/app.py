# =============================================================================
# src/dashboard/app.py — Streamlit Dashboard IDX Screener v2
# Fix: chart tidak hilang saat ganti saham di selectbox
# =============================================================================
...
SIGNAL_COLOR = {
    "Akumulasi":"#22c55e","Distribusi":"#f97316",
    "Mark Up":"#3b82f6","Mark Down":"#ef4444",
}
SIGNAL_EMOJI = {
...
# =============================================================================
# FUNGSI SCREENING — di-cache agar tidak re-run saat selectbox berubah
# =============================================================================

@st.cache_data(show_spinner=False)
def run_screening(tickers: tuple, use_cache: bool, has_foreign: bool) -> tuple[dict, pd.DataFrame]:
    """
    Cache hasil screening berdasarkan kombinasi tickers + use_cache + has_foreign.
    Hanya re-run jika parameter berubah ATAU user klik tombol (cache di-clear manual).
    Return: (ohlcv_dict, results_df)
    """
    ...
    parts = [r for name, fn in FN_MAP.items()
             if not (r := fn(ohlcv, ff)).empty]
...
with st.sidebar:
    ...
    # Universe
    universe_opt = st.selectbox("Universe Saham",
    ...
    if universe_opt == "LQ45":
        ...
        raw = st.text_area("Ticker (pisah koma)", "BBCA, BBRI, TLKM, ASII, BMRI", height=100)
    ...
    st.caption("Upload CSV foreign flow. Tanpa upload, screener tetap berjalan.")
    ...
    if uploaded_file is not None:
        try:
            ...
            if not ff.empty:
                ...
                run_screening.clear()  # clear cache agar screening ulang dengan data baru
                st.success(f"✅ {ff['ticker'].nunique()} saham dimuat")
            else:
                st.warning("⚠ File tidak terbaca — cek format")
...
if run_button:
    # Clear cache dulu agar fresh run
    run_screening.clear()
    st.session_state.screening_done = False

    with st.spinner("⏳ Mengambil data & mendeteksi sinyal..."):
        try:
            ohlcv, combined = run_screening(
                tickers=tuple(tickers),
                use_cache=use_cache,
                has_foreign=st.session_state.has_foreign,
            )
            # Filter
            if min_strength > 0 and not combined.empty:
                combined = combined[combined["strength"] >= min_strength].reset_index(drop=True)
            if show_signals and not combined.empty:
                combined = combined[combined["signal"].isin(show_signals)].reset_index(drop=True)
...
if st.session_state.has_foreign:
    st.success("✅ **Mode lengkap** — harga, volume & foreign flow tersedia.")
else:
    st.warning(
        "⚠️ **Mode harga & volume** — upload CSV asing di sidebar untuk hasil lebih akurat."
    )
st.markdown("---")
...
# =============================================================================
# HASIL SCREENING
# Ambil dari session state — TIDAK berubah saat selectbox di-klik
# =============================================================================
...
with tab1:
    ...
    if no_foreign:
        st.info("⚠️ Mode harga & volume — strength maksimal 70 untuk Akumulasi/Distribusi.")
...
with tab2:
    ...
    # key unik + index agar tidak reset saat re-run
    ...
    if selected and selected in df["ticker"].values:
        ...
        if "data_asing" in row and not row["data_asing"] and row["signal"] in ["Akumulasi","Distribusi"]:
            st.warning("⚠️ Sinyal ini tanpa konfirmasi data asing — cek RTI/Stockbit sebelum keputusan.")
        ...
        # Chart — ambil dari ohlcv_cache di session state
        # Tidak akan hilang karena ohlcv_cache disimpan di session state, bukan dicompute ulang
...
with tab3:
    ...
    if st.session_state.has_foreign:
        ...
        st.warning("⚠️ Data harga & volume saja")
...
Nilai dalam **Rupiah**. Nama kolom fleksibel — parser otomatis detect.
""")
